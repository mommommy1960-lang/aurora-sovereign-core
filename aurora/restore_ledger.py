...
class RestoreLedger:
    ...
    def set_trusted_head(self, head: str) -> None:
        with self._connect() as db:
            ...
            db.execute(
                "INSERT INTO control VALUES(1, ?) ON CONFLICT(singleton) DO UPDATE SET trusted_head=excluded.trusted_head",
                (head,),
            )
            db.commit()
        ...
        with closing(self._connect()) as db:
            try:
                ...
                db.execute(
                    ...
                    (nonce, snapshot_hash, destination, source_head, key_epoch, attempt),
                )
        ...
        *,
        ...
        with closing(self._connect()) as db:
            try:
                ...
                db.execute("INSERT INTO restore_audit VALUES(?, ?)", (nonce, decision))
                db.execute(
                    "INSERT INTO restore_jobs VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        generation,
                        nonce,
                        snapshot_path,
                ...
                db.rollback()
                raise RestoreRejected("generation or approval already used") from exc
            except BaseException:
                db.rollback()
                raise
    ...
    def advance(self, generation: str, expected: str, target: str) -> None:
        ...
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            changed = db.execute(
                "UPDATE restore_jobs SET stage=? WHERE generation=? AND stage=?",
                (target, generation, expected),
            ).rowcount
            if changed != 1:
                db.rollback()
                raise RestoreRejected("restore stage conflict")
            db.commit()
    ...
    def publish(self, generation: str) -> bool:
        ...
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            job = db.execute(
                "SELECT stage, parent_generation FROM restore_jobs WHERE generation=?",
                (generation,),
            ).fetchone()
            visible = db.execute(
                "SELECT visible_generation FROM publication WHERE singleton=1"
            ).fetchone()[0]
            if visible == generation:
                db.execute(
                    "UPDATE restore_jobs SET stage='PUBLISHED' WHERE generation=?",
                    (generation,),
                )
                db.commit()
                return False
            if job is None or job[0] != "READY" or visible != job[1]:
                db.rollback()
                raise RestoreRejected("publication requires reconciliation")
            db.execute(
                "UPDATE publication SET visible_generation=? WHERE singleton=1",
                (generation,),
            )
            db.execute(
                "UPDATE restore_jobs SET stage='PUBLISHED' WHERE generation=?",
    ...
    def stall(
        ...
        *,
        ...
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            job = db.execute(
                "SELECT stage, snapshot_hash FROM restore_jobs WHERE generation=?",
                (generation,),
            ).fetchone()
            ...
            db.execute(
                "INSERT INTO restore_failures VALUES(?, ?, ?, ?)",
                (generation, reason, expected_hash, actual_hash),
            )
            db.execute(
                "INSERT INTO attention_outbox(generation, reason) VALUES(?, ?)",
                (generation, reason),
            )
            db.execute(
                "UPDATE restore_jobs SET stage='STALLED' WHERE generation=?",
                (generation,),
            )
            db.commit()
    ...
    def pending_attention_outbox(self):
        with self._connect() as db:
            return db.execute(
                "SELECT generation, reason FROM attention_outbox WHERE delivered=0 ORDER BY generation"
            ).fetchall()

    def mark_attention_delivered(self, generation: str) -> None:
        with self._connect() as db:
            changed = db.execute(
                "UPDATE attention_outbox SET delivered=1 WHERE generation=? AND delivered=0",
                (generation,),
            ).rowcount
            if changed != 1:
    ...
    def failure(self, generation: str):
        with self._connect() as db:
            return db.execute(
                "SELECT reason, expected_hash, actual_hash FROM restore_failures WHERE generation=?",
                (generation,),
            ).fetchone()
    ...
    def job(self, generation: str):
        with self._connect() as db:
            return db.execute(
                "SELECT snapshot_path, snapshot_hash, destination, source_head, key_epoch, parent_generation, stage FROM restore_jobs WHERE generation=?",
                (generation,),
            ).fetchone()
